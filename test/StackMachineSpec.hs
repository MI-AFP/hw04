...
spec = do
  describe "runProgram (input/output and constants)" $ do
    it "runs WR to put on output from top of stack" $ do
      ...
      runProgram (RD $. TV 10 $. WR $. EOP) (Seq.singleton 7) `shouldBe` Seq.fromList [10, 7]
      runProgram (RD $. RD $. WR $. EOP) (Seq.fromList [1, 2, 3]) `shouldBe` Seq.fromList [2, 1]
  ...
  describe "runProgram (complex programs)" $ do
    it "can sum list of number in cycle (first is length)" $ do
      runProgram Progs.sumList (Seq.fromList [1..10]) `shouldBe` Seq.singleton 55
      runProgram Progs.sumList (Seq.fromList [-5..5]) `shouldBe` Seq.singleton 0
      runProgram Progs.sumList Seq.empty `shouldBe` Seq.singleton 0
    it "can compute GCD based on euclidean algorithm" $ do
      ...
      runProgram Progs.gcd (Seq.fromList [0, 1]) `shouldBe` Seq.singleton 0
